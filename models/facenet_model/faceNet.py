...
class FaceNet:
    ...
    def __init__(
        ...
        providers = providers if ort.get_device(
        ) == "GPU" and not force_cpu else providers[::-1]
    ...
    def detect_save_faces(self, image: np.ndarray, output_dir: str = "faces"):
        ...
        """
        face_crops = [image[t:b, l:r]
                      for t, l, b, r in self.detector(image, return_tlbr=True)]
    ...
    def load_anchors(self, faces_path: str):
        ...
        for face_path in stow.ls(faces_path):
            anchors[stow.basename(face_path)] = self.encode(
                cv2.imread(face_path.path))
    ...
    def encode(self, face_image: np.ndarray) -> np.ndarray:
        ...
        encode = self.ort_sess.run(
            None, {self.ort_sess._inputs_meta[0].name: np.expand_dims(face, axis=0)})[0][0]
    ...
    def draw(self, image: np.ndarray, face_crops: dict):
        ...
        for value in face_crops.values():
            ...
            cv2.putText(image, stow.name(
                value['name']), (l, t - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, self.color, self.thickness)
    ...
    def __call__(self, frame: np.ndarray) -> np.ndarray:
        ...
        """
        face_crops = {index: {"name": "Unknown", "tlbr": tlbr}
                      for index, tlbr in enumerate(self.detector(frame, return_tlbr=True))}
